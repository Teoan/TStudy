 ...
 * @description 开发环境工单处理器
 */
@Slf4j
public class DevOrderHandleServiceImpl extends OrderHandleServiceImpl {

    @Override
    public void handelOpenOrder(Order order) {
        log.info("开发环境开始处理开通工单，工单标题[{}],工单类型[{}]",order.getTitle(),order.getOrderTpye().name());
    }

    @Override
    public void handelAdjustOrder(Order order) {
        log.info("开发环境开始处理调整工单，工单标题[{}],工单类型[{}]",order.getTitle(),order.getOrderTpye().name());
    }

    @Override
    public void handelCancelOrder(Order order) {
        log.info("开发环境开始处理取消工单，工单标题[{}],工单类型[{}]",order.getTitle(),order.getOrderTpye().name());
    }
}
